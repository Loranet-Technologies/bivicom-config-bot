#!/usr/bin/env python3
"""
Unified Bivicom Configuration Bot (No Reboot Version)
===================================================

This unified script combines all functionality from Script No. 1, 2, 3, and Master Bot:
1. Network Discovery & SSH Connection
2. UCI Backup Creation
3. Network Configuration (WAN DHCP, LAN Static)
4. Connectivity Verification
5. Curl Installation
6. Infrastructure Deployment
7. Installation Verification
8. Tailscale Setup
9. UCI Configuration Restore
10. Master Bot Orchestration

NO REBOOT - All changes applied without device restart
...
class UnifiedBivicomBot:
    def __init__(self, config_file: str = "bot_config.json"):
        ...
        # Default configuration with updated flow parameters (NO REBOOT)
        self.default_config = {
            ...
            "network_configuration": {
                ...
                "wan_protocol": "dhcp",
                "lan_protocol": "static",  # Updated to static
                "ssh_ready_delay": 30,
                "config_wait_time": 5,
    ...
    def configure_network_settings(self, ssh, ip: str) -> bool:
        """Configure WAN for DHCP and LAN for static mode (NO REBOOT)"""
        self.log(f"Configuring network settings on {ip} (NO REBOOT)")
        
        try:
            # WAN Configuration (DHCP)
            wan_commands = [
                "sudo uci set network.wan.proto='dhcp'",
                "sudo uci set network.wan.ifname='eth0'",
                "sudo uci set network.wan.mtu=1500"
            ]
            
            # LAN Configuration (Static) - UPDATED
            lan_commands = [
                "sudo uci set network.lan.proto='static'",  # Changed from DHCP
                "sudo uci set network.lan.ifname='eth1'",
                "sudo uci set network.lan.ipaddr='192.168.1.1'",  # Added static IP
                "sudo uci set network.lan.netmask='255.255.255.0'"  # Added netmask
            ]
            
            # Apply configuration (NO REBOOT - just reload)
            apply_commands = [
                "sudo uci commit network",
                "sudo /etc/init.d/network reload"  # Only reload, no reboot
            ]
            ...
            # Apply configuration (NO REBOOT)
            self.log(f"[{ip}] Applying network configuration (NO REBOOT)...")
            ...
            self.log(f"Network configuration completed on {ip} (NO REBOOT)", "SUCCESS")
            return True
    ...
    def check_wan_connectivity(self, ssh, ip: str) -> bool:
        ...
        try:
            ...
            if wan_ip:
                ...
                self.log("WAN interface has no IP address", "WARNING")
                return False
    ...
    def restore_uci_backup(self, ssh, ip: str) -> bool:
        """Restore UCI configuration from backup"""
        ...
        try:
            ...
            restore_commands = [
                f"sudo uci restore {self.backup_path}",
                "sudo uci commit",
                "sudo /etc/init.d/network reload"  # Only reload, no reboot
            ]
            ...
            for cmd in restore_commands:
                ...
                if exit_status != 0:
                    error_output = stderr.read().decode('utf-8').strip()
                    self.log(f"[{ip}] Restore command failed: {cmd} - {error_output}", "WARNING")
                else:
                    self.log(f"[{ip}] Restore command successful: {cmd}", "SUCCESS")
    ...
    def run_complete_deployment_cycle(self) -> bool:
        ...
        self.log("=" * 80)
        self.log(f"UNIFIED BIVICOM BOT CYCLE #{self.cycle_count + 1}: COMPLETE DEPLOYMENT (NO REBOOT)")
        self.log("=" * 80)
        ...
        try:
            ...
            # Step 6: Configure network settings (NO REBOOT)
            self.log("Step 6: Configuring network settings (NO REBOOT)")
            if not self.configure_network_settings(ssh, target_ip):
            ...
            restore_ok = self.restore_uci_backup(ssh, target_ip)
            if not restore_ok:
            ...
            # Success!
            self.log("=" * 80)
            self.log("UNIFIED BIVICOM BOT CYCLE COMPLETED SUCCESSFULLY! (NO REBOOT)", "SUCCESS")
            self.log("=" * 80)
            self.log("All deployment steps completed:")
            self.log("- UCI backup created")
            self.log("- Network configured (WAN DHCP, LAN Static) - NO REBOOT")
            ...
            self.log("- UCI configuration restored - NO REBOOT")
    ...
    def run_forever_mode(self):
        """Run the bot in forever mode (NO REBOOT)"""
        self.log("=" * 80)
        self.log("UNIFIED BIVICOM BOT STARTING - RUNNING FOREVER MODE (NO REBOOT)")
        ...
        while not self.shutdown_requested:
            ...
            success = self.run_complete_deployment_cycle()
    ...
    def run_single_cycle(self):
        ...
        self.log("=" * 80)
        self.log("UNIFIED BIVICOM BOT - SINGLE CYCLE MODE (NO REBOOT)")
        self.log("=" * 80)
        
        success = self.run_complete_deployment_cycle()
        
        if success:
            self.log("Single cycle completed successfully! (NO REBOOT)", "SUCCESS")
            return True
        else:
...
def main():
    ...
    print("9. UCI Configuration Restore - NO REBOOT")
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--single":
        mode = "single"
        print("Running in SINGLE CYCLE mode (NO REBOOT)")
    else:
        mode = "forever"
        print("Running in FOREVER mode (NO REBOOT) - press Ctrl+C to stop")
